...
│
├── 📹 视频格式转换
│   ├── 格式支持：MP4、AVI、MKV、MOV、WebM、FLV、WMV、TS
│   ├── 编码选择：H.264、H.265、VP9、AV1、ProRes
│   ├── 质量控制：CRF 参数调节、预设选择
│   ├── 分辨率调节：4K、2K、1080p、720p、自定义
│   ├── 帧率/比特率调节
│   └── 像素格式选择
│
├── 🎵 音频处理
│   ├── 音频提取：从视频提取音轨
│   ├── 格式转换：MP3、AAC、WAV、FLAC、OGG、OPUS
│   ├── 音质设置：比特率、采样率
│   ├── 音量调节、淡入淡出
│   ├── 降噪处理、均衡器调节
│   └── 声道处理
...
│   ├── MediaTools/    # 媒体工具
│   │   ├── index.tsx         # 媒体工具主页
│   │   ├── FormatConvert.tsx # 格式转换
│   │   ├── AudioProcess.tsx  # 音频处理
│   │   ├── VideoProcess.tsx  # 视频整合
│   │   ├── VideoEdit.tsx     # 视频处理
│   │   ├── FolderProcess.tsx # 文件夹处理
│   │   ├── AdvancedTools.tsx # 高级工具
│   │   ├── ImageFormatConvert.tsx # 图片格式转换
│   │   ├── IcoConverter.tsx  # ICO转换
│   │   └── CommandBuilder.tsx # 命令构建器
│   ├── Models/        # 模型管理
...
<details>
<summary><b>Q: 媒体工具箱如何使用？</b></summary>

1. 首先在设置页面配置 FFmpeg 可执行文件路径
2. 选择对应的标签页（格式转换、音频处理、视频处理等）
3. 拖拽或点击导入媒体文件
4. 设置输出参数并开始处理
</details>

<details>
<summary><b>Q: 视频整合功能有什么用？</b></summary>

视频整合功能可以：
1. 对比多个视频的参数信息（分辨率、帧率、码率等）
2. 查看处理前后视频的差异
3. 与视频处理标签页联动，自动导入处理前后的视频进行对比
</details>
