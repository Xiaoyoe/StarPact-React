...
export class IndexedDBStorage {
  ...
  private constructor() {
    ...
    this.dbVersion = 9; // 增加版本号，添加 set-background 表
    this.initDatabase();
  }
  ...
  private handleDatabaseUpgrade = (event: IDBVersionChangeEvent): void => {
    ...
    // 创建 Ollama 模型文件存储
    if (!db.objectStoreNames.contains('ollama-model')) {
      db.createObjectStore('ollama-model', { keyPath: 'id' });
    }
    ...
    // 创建自定义壁纸存储
    if (!db.objectStoreNames.contains('set-background')) {
      db.createObjectStore('set-background', { keyPath: 'id' });
    }
